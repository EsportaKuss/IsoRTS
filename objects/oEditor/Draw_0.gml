...
#region USING LIST
		...
		decor		= list[| e_tile_data.decor_index];
		//land		= list [| e_tile_data.land_index];
				...
				if not  display_all_heights and draw_height <= current_height or (display_all_heights == true)
				{
					...
					if draw_height == height
					{
						...
						*/
						...
						place_decor(e_tile_data.decor_index, list, draw_x, draw_y, 0,0,col);
						if decor == 0 then
						place_decor(e_tile_data.land_index, list,draw_x,draw_y,-16,7,col);
					}
					//lands
		...
		if xx == grid_x and yy == grid_y
		{
			for (var draw_height = 0; draw_height <= current_height; draw_height++)
			{
				...
				//DRAW DECOR
				draw_decor(e_tile_data.decor_index,0,0);
				draw_decor(e_tile_data.land_index,-16,+7);
				//DRAW LANDING
				/*
				if current_part == e_tile_data.land_index
				{
